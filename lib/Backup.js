...
module.exports = class Backup {
    ...
    constructor(server, options) {
        ...
        this.backupTimeFormats = { minute: "y-o-H-m", hour: "y-o-H", day: "y-o", week: "y-W", month: "y-L", year: "y" };
    ...
    async createArchive() {
        ...
        for (let i=0; i<self.pending.length; i++) {
            const filename = self.archiveDir + '/' + self.pending[i] + '_' + DateTime.now().toFormat("yyyy-LL-dd_HH-mm-ss") + '_' + this.hostname + '.zip';
    ...
    getPending() {
        ...
        for (let type in this.options.backupsPerServer) {
            ...
            for (let i=0; i<this.backups[type].length; i++) {
                ...
                const mtimeFormat = DateTime.fromMillis(this.backups[type][i].mtime).toFormat(this.backupTimeFormats[type]);
    ...
    deleteOldBackups() {
        ...
        for (var type in this.backups) {
            ...
            let typeMaxBackups = Math.max(this.options.backupsPerServer[type] || 0, 0);
            ...
            while (this.backups[type].length > typeMaxBackups) {
                ...
                // Delete backup
                console.log('Delete backup: "' + backup.filename + '"');
    ...
    async start() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            // Create directories
            [self.cacheDir, self.hostDir, self.filesDir, self.backupDir, self.archiveDir].forEach(dir => {
