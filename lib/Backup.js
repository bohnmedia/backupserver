...
module.exports = class Backup {
    ...
    getBackups() {
        ...
        const backups = { minute: [], hour: [], day: [], week: [], month: [], year: [] };
    ...
    getPending() {
        ...
        for (let type in this.options.backupsPerServer) {
            ...
            const nowFormat = now.toFormat(nowType[type]);

            // Check if backup already exists
            for (let i=0; i<this.backups[type].length; i++) {

                // Convert mtime (in milliseconds) to string for comparion
                const mtimeFormat = DateTime.fromMillis(this.backups[type][i].mtime).toFormat(nowType[type]);
